...
- 以 lastOptimizedPrompt 的结构为准：如果它是结构化 JSON 或稳定的 JSON 风格对象，输出必须仍为严格 JSON；如果它是自然语言，才输出自然语言
- 即使 iterateInput 没有提到 JSON，也要保持 JSON 输出；不要因为用户口语化表达就把结构化内容改写成 prose
...
## 输出要求
- 若 lastOptimizedPrompt 为自然语言：直接输出新的“优化后图像提示词”（自然语言、纯文本）
- 若 lastOptimizedPrompt 本身为结构化 JSON：直接输出严格 JSON；不要添加解释、标题、代码块或 Markdown，也不要改写成自然语言段落
- 禁止添加任何前缀或解释；仅输出结果文本
- 保持可读性与可执行性
- 若输入为结构化 JSON：优先沿用原有结构与键语义，并保留所有原始占位符逐字不变
...
重要补充：
- 如果 lastOptimizedPrompt 本身已经是结构化 JSON 或包含双花括号占位符，则输出结果必须继续保持 JSON 结构，并保留所有占位符逐字不变
